...
bool crn_comp::alias_images() {
  for (uint face_index = 0; face_index < m_pParams->m_faces; face_index++) {
    for (uint level_index = 0; level_index < m_pParams->m_levels; level_index++) {
	  ...
	  const uint width = uniformMips ? m_pParams->m_width : math::maximum(1U, m_pParams->m_width >> level_index);
	  const uint height = uniformMips ? m_pParams->m_height : math::maximum(1U, m_pParams->m_height >> level_index);
	  //UE4_END
  ...
  for (uint level = 0; level < m_pParams->m_levels; level++) {
    //UE4_BEGIN
    const bool uniformMips = m_pParams->get_flag(cCrnCompFlagUniformMips);
	const uint width = uniformMips ? m_pParams->m_width : math::maximum(1U, m_pParams->m_width >> level);
	const uint height = uniformMips ? m_pParams->m_height : math::maximum(1U, m_pParams->m_height >> level);
	uint blockHeight = ((height + 7) & ~7) >> 2;
    m_levels[level].block_width = ((width + 7) & ~7) >> (m_has_etc_color_blocks ? 1 : 2);
	//UE4_END
...
bool crn_comp::create_comp_data() {
  ...
  //UE4_BEGIN
  m_crn_header.m_levels = static_cast<uint16>(m_pParams->m_levels);
  //EU4_END
  ...
  //UE4_BEGIN
  if (m_pParams->get_flag(cCrnCompFlagUniformMips))
  {
	  m_crn_header.m_flags = crnd::cCRNHeaderFlagUniformMips;
  }
  //UE4_END
  m_comp_data.clear();
