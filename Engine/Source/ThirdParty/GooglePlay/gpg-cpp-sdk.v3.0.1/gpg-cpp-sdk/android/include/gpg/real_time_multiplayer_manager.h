...
class GPG_EXPORT RealTimeMultiplayerManager {
 public:
   ...
   */
  void CreateRealTimeRoom(const gpg::RealTimeRoomConfig &config,
   ...
   */
  RealTimeRoomResponse CreateRealTimeRoomBlocking(
      const RealTimeRoomConfig &config, IRealTimeEventListener *listener);
   ...
   */
  RealTimeRoomResponse CreateRealTimeRoomBlocking(
      const RealTimeRoomConfig &config, RealTimeEventListenerHelper helper);
   ...
   */
  typedef std::function<void(const ResponseStatus &)> LeaveRoomCallback;
   ...
   */
  void SendReliableMessage(const RealTimeRoom &room,
   ...
   * faster than sending data reliably and should be preferred if possible.
   */
  void SendUnreliableMessageToOthers(const RealTimeRoom &room,
                                     std::vector<uint8_t> data);
   ...
   */
  typedef std::function<void(const RoomInboxUIResponse &)> RoomInboxUICallback;
   ...
   */
  void ShowWaitingRoomUI(const RealTimeRoom &room,
   ...
   */
  WaitingRoomUIResponse ShowWaitingRoomUIBlocking(
      const RealTimeRoom &room, uint32_t min_participants_to_start);
 ...
 private:
  ...
  RealTimeMultiplayerManager(const RealTimeMultiplayerManager &) = delete;
  RealTimeMultiplayerManager &operator=(const RealTimeMultiplayerManager &) =
      delete;
