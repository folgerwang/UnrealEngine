...
///--BEGIN_SDK_REMOVE
	...
	ovrControllerCaps_ModelGearVRPlus			= 0x00000100,	// controller for Gear VR Plus devices
	ovrControllerCaps_HasHapticVibration		= 0x00000200,	// Controller supports haptic vibration
///--END_SDK_REMOVE
...
///--BEGIN_SDK_REMOVE
/// Specifies haptic capabilities of a devices
typedef enum ovrHapticCaps_
{
	ovrHapticCaps_HasSimpleHapticVibration = 0x00000001,
	ovrHapticCaps_HasBufferedHapticVibration = 0x00000002,

	ovrHapticCaps_EnumSize = 0x7fffffff
} ovrHapticCaps;

/// This header starts all ovrHapticCapabilities structures. It should only hold fields
/// that are common to all haptic devices
typedef struct ovrHapticCapabilityHeader_
{
	ovrControllerType	Type;

	/// A unique ID for the input device
	ovrDeviceID			DeviceID;
} ovrHapticCapabilityHeader;

/// Details about the Haptic capabilities of the device
typedef struct ovrHapticCapabilities_
{
	ovrHapticCapabilityHeader	Header;

	/// Mask of controller capabilities described by ovrControllerCapabilities
	uint32_t					HapticCapabilities;
...
///--BEGIN_SDK_REMOVE
/// Returns the haptic capabilities of the input device for the corresponding device ID
///
/// Input: ovr, deviceID, pointer to a capabilities structure
/// Output: Upon return the capabilities structure will contain the haptic capabilities.
OVR_VRAPI_EXPORT ovrResult vrapi_GetHapticBufferCapabilities( ovrMobile * ovr, const ovrDeviceID deviceID, ovrHapticCapabilityHeader * capsHeader );
