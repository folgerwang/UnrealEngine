...
FArchive* FFileManagerGeneric::CreateFileWriterInternal( const TCHAR* Filename, uint32 Flags, uint32 BufferSize )
{
	...
	return new FArchiveFileWriterGeneric( Handle, Filename, Handle->Tell(), Flags, BufferSize );
}
...
FArchiveFileWriterGeneric::FArchiveFileWriterGeneric( IFileHandle* InHandle, const TCHAR* InFilename, int64 InPos, uint32 InFlags, uint32 InBufferSize )
	: Filename( InFilename )
	, Flags( InFlags )
	, Pos( InPos )
	, BufferCount( 0 )
