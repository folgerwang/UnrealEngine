...
ImmediatePhysics::FActorHandle* FSimulation::InsertActorData(const FActor& InActor, const FActorData& InData)
{
	FActor* NewActor = new(Actors) FActor(InActor);
	const uint32 ActorDataIndex = Actors.Num() - 1;
	...
	PxSolverBodyData* NewSolverBodyData = new (SolverBodiesData) PxSolverBodyData();
	RigidBodiesData.Add(InData.RigidBodyData);

	immediate::PxRigidBodyData& InternalData = RigidBodiesData.Last();

	NewActorHandle->ActorToBody = FTransform::Identity; // #PHYS2 should be the centre of mass of the actor
	InternalData.body2World = U2PTransform(NewActorHandle->ActorToBody * InData.InitialTransform);

	PendingAcceleration.AddZeroed();
	KinematicTargets.AddZeroed();

	const bool bDynamic = !InData.bStatic && !InData.bKinematic;
	if(bDynamic)
	{
		// Dynamic object
	...
	{
		// Static object
		immediate::PxConstructStaticSolverBody(InternalData.body2World, *NewSolverBodyData);
	}

	ValidateArrays();

	//We sort entities so that dynamics are first, kinematics second, and static third
	if(bDynamic)	//dynamic first
	{
		if(NumSimulatedBodies < ActorDataIndex)	//There's at least one kinematic/static entity so swap
		{
			const int32 FirstNonSimulatedIndex = NumSimulatedBodies;
	...
	else if(InData.bKinematic)  //then kinematics
	{
		const uint32 NumMobileBodies = NumKinematicBodies + NumSimulatedBodies;
		if(NumMobileBodies < ActorDataIndex)	//There's at least one static entity so swap
		{
...
void FSimulation::RemoveActor(ImmediatePhysics::FActorHandle* InHandle)
{
	int32 DataIndex = InHandle->ActorDataIndex;

#if WITH_PHYSX
	check(Actors.IsValidIndex(DataIndex));
	check(DataIndex >= 0);

	bool bDynamic = (uint32)DataIndex < NumSimulatedBodies;
	bool bKinematic = !bDynamic && (uint32)DataIndex < NumKinematicBodies;
	...
	KinematicTargets.RemoveAt(DataIndex);

	// Starting where we removed the actor we need to update any handles beyond it
	const int32 NumTotalHandles = ActorHandles.Num();
	for(int32 CurrIndex = DataIndex; CurrIndex < NumTotalHandles; ++CurrIndex)
	{
		ActorHandles[CurrIndex]->ActorDataIndex--;
	}
	...
	if(bDynamic)
	{
		...
		// This is used for RBAN LOD optimizations, in normal simulations this should be identical to NumSimulatedBodies
...
void FSimulation::ConstructSolverBodies(float DeltaTime, const FVector& Gravity)
{
	...
	{
		const FImmediateKinematicTarget& KinematicTarget = KinematicTargets[KinematicBodyIdx];
...
ImmediatePhysics::FActorData CreateActorData(const FActorCreationParams& InParams)
{
	FActorData OutData;
	...
	RigidData.invMass = 0.0f;
	RigidData.angularVelocity = PxVec3(PxZero);
	RigidData.maxDepenetrationVelocity = PX_MAX_F32;
	RigidData.invInertia = PxVec3(PxZero);
	RigidData.maxContactImpulse = PX_MAX_F32;
	RigidData.linearDamping = 0.0f;
	RigidData.angularDamping = 0.0f;
	...
	OutData.bKinematic = false; //#PHYS2 Add kinematic support
