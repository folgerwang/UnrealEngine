...
bool UEdGraphSchema::DoesDefaultValueMatchAutogenerated(const UEdGraphPin& InPin) const
{
	...
	FText AutogeneratedDefaultValueAsText;
	if (!InPin.DefaultTextValue.IsEmpty() && FTextStringHelper::ReadFromBuffer(*InPin.AutogeneratedDefaultValue, AutogeneratedDefaultValueAsText))
	{
		return UTextProperty::Identical_Implementation(AutogeneratedDefaultValueAsText, InPin.DefaultTextValue, 0);
	}
	else
	{
