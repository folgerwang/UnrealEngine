...
namespace GLTF
{
	...
	void FPrimitive::GetPositions(TArray<FVector>& Buffer) const
	{
		Position.GetVec3Array(Buffer);
		for (FVector& Vec : Buffer)
		{
			Vec = GLTF::ConvertVec3(Vec);
		}
	}

	void FPrimitive::GetNormals(TArray<FVector>& Buffer) const
	{
		Normal.GetVec3Array(Buffer);
		for (FVector& Vec : Buffer)
		{
			Vec = GLTF::ConvertVec3(Vec);
		}
