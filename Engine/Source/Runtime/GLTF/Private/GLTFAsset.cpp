...
namespace GLTF
{
	...
	FString FAsset::GetName(const FString& FileName) const
	{
		FString SceneName = FPaths::GetBaseFilename(FileName);
		if (SceneName.ToLower() == TEXT("scene"))
		{
			// change name, try to see if asset title is given
			if (const FMetadata::FExtraData* Extra = Metadata.GetExtraData(TEXT("title")))
			{
				SceneName = Extra->Value;
			}
			else
			{
				SceneName = FPaths::GetBaseFilename(FPaths::GetPath(FileName));
			}
		}
		return SceneName;
	}
	...
	void FAsset::GenerateNames(const FString& Prefix)
	{
		...
		GLTF::GenerateNames(Prefix + TEXT("_node_"), Nodes);
		...
		for (FLight& Light : Lights)
		{
			if (Light.Name.IsEmpty())
				Light.Name = TEXT("light_") + Light.Node.Name;  // cant be empty
		}
