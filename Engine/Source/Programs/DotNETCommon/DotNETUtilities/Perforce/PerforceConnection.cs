...
/*
			using(PerforceChildProcess ChildProcess = new PerforceChildProcess("{0} print \"{1}\"", GlobalOptions, FileSpec))
			{
				PerforceRecordType Type;
				if(ChildProcess.TryBeginRecord(out Type))
				{
					for(;;)
					{
						if(Type != PerforceRecordType.Stat)
						{
							throw new PerforceException("Unexpected record type: {0}", Type);
						}

						Dictionary<string, object> Fields = new Dictionary<string, object>();
						ChildProcess.ReadFields(Fields);

						string OutputFile = Path.Combine("D:\\WSTEST\\Content", Fields["depotFile"].ToString().Replace('/', '+'));
//						Console.WriteLine(OutputFile);
						...
						using(FileStream OutputStream = File.Open(OutputFile, FileMode.Create, FileAccess.Write, FileShare.Read))
						{
							for(;;)
							{
								if(!ChildProcess.TryBeginRecord(out Type))
								...
								if(Type != PerforceRecordType.Text && Type != PerforceRecordType.Binary)
								{
									break;
								}

								string Key;
								if(!ChildProcess.TryReadKey(out Key))
								{
									throw new PerforceException("Foo");
								}
								if(Key != "data")
								{
									throw new PerforceException("Bar");
								}
								ChildProcess.ReadDataValue(OutputStream);
...
/*				while(!Process.IsEndOfFile())
				{
					// Check that a dictionary follows
					byte Temp = Process.ReadByte();
					if(Temp != '{')
					{
						throw new PerforceException("Unexpected data while parsing marshalled output - expected '{'");
					}
					...
					for(;;)
					{
						...
						byte KeyFieldType = Process.ReadByte();
						if(KeyFieldType == '0')
						{
							break;
						}
						...
						// Read the key
						string Key = Process.ReadString();

						// Read the value type.
						byte ValueFieldType = Process.ReadByte();
						if(ValueFieldType == 'i')
						{
							// An integer
							string Value = Process.ReadInt32().ToString();
...
//							Record.Add(new KeyValuePair<string, object>(Key, Value));
						...
						else if(ValueFieldType == 's')
						{
							...
							if(Key == "data")
							{
								...
								Process.CopyData(OutputData);
								Console.WriteLine("{0}={1} bytes", Key, OutputData.Length);
							}
							else
							{
								string Value = Process.ReadString();
								Console.WriteLine("{0}={1}", Key, Value);
							}
						}
						else
