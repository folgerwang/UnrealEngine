...
namespace AutomationTool
{
	...
	public partial class P4Connection
	{
		...
		public string[] P4Print(string CommandLine, bool bQuiet = false)
		{
			if (!bQuiet)
			{
				CommandUtils.LogInformation("P4Print({0})", CommandLine);
			}
		...
		public string[] Annotate(string CommandLine, bool bQuiet = false)
		{
			if (!bQuiet)
			{
				CommandUtils.LogInformation("Annotate({0})", CommandLine);
			}
		...
		public DiffChange[] Diff2(string DepotFilename, int RevA, int RevB, bool bQuiet = false)
		{
			if (!bQuiet)
			{
				CommandUtils.LogInformation("Diff2({0}, {1}, {2})", DepotFilename, RevA, RevB);
			}
		...
		{
			CommandUtils.LogInformation("Timelapse({0}#{1})", DepotFilename, (MaxRevision == Int32.MaxValue) ? "head" : MaxRevision.ToString());
		...
		{
			CommandUtils.LogInformation("Blame({0})", DepotFilename);
			// Get the file history.
			var FileHistory = Filelog(DepotFilename, bQuiet: true);
