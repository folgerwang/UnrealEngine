...
namespace UnrealBuildTool
{
	abstract class MSBuildProjectFile : ProjectFile
	{
		...
		public abstract void MakeProjectPlatformAndConfigurationNames(UnrealTargetPlatform Platform, UnrealTargetConfiguration Configuration, string TargetConfigurationName, PlatformProjectGeneratorCollection PlatformProjectGenerators, out string ProjectPlatformName, out string ProjectConfigurationName);
	...
	class VCProjectFile : MSBuildProjectFile
	{
						...
						if ((BuildPlatform != null) && (BuildPlatform.HasRequiredSDKsInstalled() == SDKStatus.Valid))
						{
							...
							foreach (ProjectTarget ProjectTarget in ProjectTargets)
							{
								if (IsValidProjectPlatformAndConfiguration(ProjectTarget, Platform, Configuration, PlatformProjectGenerators))
								{
									...
									MakeProjectPlatformAndConfigurationNames(Platform, Configuration, ProjectTarget.TargetRules.Type.ToString(), PlatformProjectGenerators, out ProjectPlatformName, out ProjectConfigurationName);
		...
		public override bool WriteProjectFile(List<UnrealTargetPlatform> InPlatforms, List<UnrealTargetConfiguration> InConfigurations, PlatformProjectGeneratorCollection PlatformProjectGenerators)
		{
			...
			foreach (ProjectConfigAndTargetCombination Combination in ProjectConfigAndTargetCombinations)
			{
				WriteConfiguration(ProjectName, Combination, PlatformProjectGenerators, VCProjectFileContent, bGenerateUserFileContent ? VCUserFileContent : null);
			}
		...
		private void WriteConfiguration(string ProjectName, ProjectConfigAndTargetCombination Combination, PlatformProjectGeneratorCollection PlatformProjectGenerators, StringBuilder VCProjectFileContent, StringBuilder VCUserFileContent)
		{
