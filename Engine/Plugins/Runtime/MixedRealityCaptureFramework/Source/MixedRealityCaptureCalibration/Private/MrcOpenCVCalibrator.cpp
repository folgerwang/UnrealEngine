...
UMrcOpenCVCalibrator* UMrcOpenCVCalibrator::CreateCalibrator(int32 BoardWidth /*= 7*/, int32 BoardHeight /*= 5*/, float InSquareSize /*= 3.f*/)
{
	UMrcOpenCVCalibrator* Result = NewObject<UMrcOpenCVCalibrator>(UMrcOpenCVCalibrator::StaticClass());
	Result->Reset(BoardWidth, BoardHeight, InSquareSize);
	return Result;
}

void UMrcOpenCVCalibrator::Reset(int32 BoardWidth, int32 BoardHeight, float InSquareSize/*=3.f*/)
{
...
bool UMrcOpenCVCalibrator::FeedRenderTarget(UTextureRenderTarget2D* TexRT)
{
#if WITH_OPENCV
	if (TexRT)
	{
...
bool UMrcOpenCVCalibrator::FeedImage(const FString& FilePath)
{
#if WITH_OPENCV
...
#if WITH_OPENCV
bool UMrcOpenCVCalibrator::Feed(const Mat& Image)
{
...
bool UMrcOpenCVCalibrator::CalculateLensParameters(struct FMrcLensDistortion& OutLensDistortion, float& OutHFOV, float& OutVFOV, float& OutAspectRatio, float& OutMarginOfError)
{
#if WITH_OPENCV
	...
	OutMarginOfError = FLT_MAX;
	{
		...
		OutMarginOfError = (float)calibrateCamera(ObjectPoints, ImagePoints, ImageSize, CameraMatrix, DistCoeffs, Rvecs, Tvecs);
