...
OPENCV_INCLUDES_END
#endif
#include "MrcLensDistortion.generated.h"

USTRUCT(BlueprintType)
struct MIXEDREALITYCAPTUREFRAMEWORK_API FMrcLensDistortion
{
...
public:
...
#if WITH_OPENCV
	FMrcLensDistortion(const cv::Mat& DistCoeffs, const cv::Mat& CameraMatrix);
	...
	bool operator == (const FMrcLensDistortion& Other) const
	{
		return (
			K1 == Other.K1 &&
			K2 == Other.K2 &&
	...
	bool IsSet() const
	{
		return *this != FMrcLensDistortion();
	}

	/** Radial parameter #1. */
	UPROPERTY(Interp, EditAnywhere, BlueprintReadWrite, Category = "MixedRealityCapture|LensCalibration")
	float K1;
	 ...
	 * @param UndistortedFocalRatio the focal length aspect ratio of the undistorted image.
	 */
	class UTexture2D* CreateUndistortUVMap(FIntPoint ImageSize, float Alpha, float& UndistortedHFOV, float& UndistortedVFOV, float& UndistortedFocalRatio);
