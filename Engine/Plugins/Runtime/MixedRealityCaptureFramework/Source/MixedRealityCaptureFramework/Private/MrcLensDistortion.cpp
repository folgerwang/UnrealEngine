...
FMrcLensDistortion::FMrcLensDistortion(const cv::Mat & DistCoeffs, const cv::Mat & CameraMatrix)
{
...
Mat FMrcLensDistortion::GetDistCoeffs()
{
...
Mat FMrcLensDistortion::GetCameraMatrix()
{
...
UTexture2D * FMrcLensDistortion::CreateUndistortUVMap(FIntPoint ImageSize, float Alpha, float & UndistortedHFOV, float & UndistortedVFOV, float & UndistortedFocalRatio)
{
#if WITH_OPENCV
	if (IsSet())
	{
		...
		// Use OpenCV to generate the map
		{
			...
			UndistortedFocalRatio = AspectRatio;
		}
		...
		{
			uint16* Row = &MipData[Y * ImageSize.X * 2];
			for (int32 X = 0; X < ImageSize.X; X++)
			{
				float UOffset = Map1.at<float>(Y, X);
				UOffset -= X;
				UOffset /= ImageSize.X;

				float VOffset = Map2.at<float>(Y, X);
				VOffset -= Y;
				VOffset /= ImageSize.Y;
