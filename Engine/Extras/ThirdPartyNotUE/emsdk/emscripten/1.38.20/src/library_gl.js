...
var LibraryGL = {
  ...
  $GL: {
    ...
    createOffscreenFramebuffer: function(context) {
      ...
      // fullscreen), but it's not known where the bug lies. To be safe, only
      // use this path on Chrome, for the moment.
      // https://github.com/kripken/emscripten/pull/7588#issuecomment-443388538
      var isChrome = /Chrome/.test(navigator.userAgent) && /Google Inc/.test(navigator.vendor);
      context.defaultFboAllowBlit = isChrome;
...
//    if (context.version >= 2) {
      if (gl.blitFramebuffer && context.defaultFboAllowBlit) {
